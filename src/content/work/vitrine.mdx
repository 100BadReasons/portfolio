...
metrics:
  - label: Scripts written
    value: "14"
    method: >-
      Purpose-built for this scene: the Sheet-driven book builder, the spine-fold
      detector, in-engine text rasterisation, logo alpha extraction, record
      artwork sourcing, the node-group builder, hole detection, camera framing
      and easing solvers, cavity baking and normalisation, HDRI colour
      measurement, render A/B scoring, Figma template publishing, and revert
      capture.
  - label: New Blender techniques
    value: "12"
    method: >-
      Things I did not know before this scene — three Blender 5.2 API changes
      that break the documented 4.x patterns, Fill Curve dropping attributes,
      baking cavity to vertex colour instead of live AO, why box projection
      fans on thin edges and why grain steers on Z not X, world vs object
      texture coordinates, transmission bounce exhaustion, hide_render against
      hide_viewport, Follow Path offset maths, GPU offscreen text needing an
      ortho matrix, and matrix_world going stale after a location set.
---
...
diagnosis in exactly the wrong direction. Names are the cheapest documentation a
scene file has, and they only stay useful if they're updated the moment the thing
behind them changes.
